...
# =================================================================
# 1. DEL: PREVERJANJE ODVISNOSTI (Samodejna priprava okolja)
# =================================================================

if [ -f /usr/bin/zypper ]; then
    if ! rpm -q python3-tk >/dev/null 2>&1; then
        echo "Pripravljam TumbleTool grafično okolje (namestitev python3-tk)..."
...
class App:
    ...
    def run_install(self, s):
        ...
        if "Fedora" in s:
            cmd = f"sudo dnf install -y {p} python3-tkinter"
        elif "Ubuntu" in s or "Debian" in s:
            cmd = f"sudo apt update && sudo apt install -y {p} python3-tk"
        elif "Arch" in s:
            cmd = f"sudo pacman -Sy --noconfirm {p} tk"
        else:
            cmd = f"sudo zypper refresh && sudo zypper install -y {p} python3-tk && " \
